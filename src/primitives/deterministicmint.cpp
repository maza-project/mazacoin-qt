...
CDeterministicMint::CDeterministicMint(uint8_t nVersion, const uint32_t &nCount,
                                       const uint256 &hashSeed,
                                       const uint256 &hashSerial,
                                       const uint256 &hashPubcoin,
...
std::string CDeterministicMint::ToString() const {
    return strprintf(
        ...
        "hashseed=%s\n   hashSerial=%s\n   hashStake=%s\n   "
        "hashPubcoin=%s\n   txid=%s\n   height=%d\n   denom=%d\n   isUsed=%d\n",
        nMintVersion, nCount, hashSeed.GetHex(), hashSerial.GetHex(),
        hashStake.GetHex(), hashPubcoin.GetHex(), txid.GetHex(), nHeight, denom,
        isUsed);
}
