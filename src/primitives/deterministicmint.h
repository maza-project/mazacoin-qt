...
// struct that is safe to store essential mint data, without holding any
// information that allows for actual spending (serial, randomness, private key)
...
public:
    CDeterministicMint();
    CDeterministicMint(uint8_t nMintVersion, const uint32_t &nCount,
                       const uint256 &hashSeed, const uint256 &hashSerial,
    ...
    void SetDenomination(const libzerocoin::CoinDenomination denom) {
        this->denom = denom;
    }
