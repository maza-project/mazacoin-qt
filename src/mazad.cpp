 ...
 * experimental new digital currency called Maza (https://www.maza.org/),
 * which enables instant payments to anyone, anywhere in the world. Maza uses
...
bool AppInit(int argc, char *argv[]) {
    ...
    // Parameters
    //
    // If Qt is used, parameters/maza.conf are parsed in qt/maza.cpp's
    // main()
    ...
    try {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) &&
                !boost::algorithm::istarts_with(argv[i], "maza:"))
                fCommandLine = true;

        if (fCommandLine) {
            fprintf(stderr, "Error: There is no RPC client functionality in "
                            "mazad anymore. Use the maza-cli utility "
                            "instead.\n");
            exit(EXIT_FAILURE);
        }
        // -server defaults to true for mazad but not for the GUI so do this
        // here
