...
static UniValue getmininginfo(const Config &config,
    ...
    obj.push_back(
        Pair("difficulty_yescrypt", (double)GetDifficulty(NULL, ALGO_SLOT4)));
    obj.push_back(
        Pair("difficulty_x11", (double)GetDifficulty(NULL, ALGO_SLOT5)));
...
UniValue getauxblock(const Config &config, const JSONRPCRequest &request) {
    ...
    CBitcoinAddress address; // WARNING (mergedaddress);
    boost::shared_ptr<CReserveScript> coinbaseScript(
