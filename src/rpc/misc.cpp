...
static UniValue getinfo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total maza "
    ...
    obj.push_back(Pair("difficulty_sha256",   (double)GetDifficulty(NULL, ALGO_SLOT4)));
...
static UniValue createmultisig(const Config &config,
    ...
    if (request.fHelp || request.params.size() < 2 ||
        ...
        std::string msg =
            ...
            "are maza addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) maza address or hex-encoded public "
            "key\n"
            "       ,...\n"
            "     ]\n"
...
static UniValue echo(const Config &config, const JSONRPCRequest &request) {
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "maza-cli and the GUI. There is no server-side difference.");
