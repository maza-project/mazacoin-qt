...
bool CAuxPow::check(const uint256 &hashAuxBlock, int nChainId,
    ...
    const uint256 blockHash;//// = header.GetHash(); // XWARNING
    if (CheckMerkleBranch(blockHash, vChainMerkleBranch, nIndex) !=
    ...
    CScript::const_iterator pcHead =
        std::search(script.begin(), script.end(), UBEGIN(pchMergedMiningHeader),
                    UEND(pchMergedMiningHeader));

    CScript::const_iterator pc = std::search(
        script.begin(), script.end(), vchRootHash.begin(), vchRootHash.end());
    ...
    if (pcHead != script.end()) {
        ...
        if (pc - script.begin() > 20)
            return error("Aux POW chain merkle root must start in the first 20 "
    ...
    pc += vchRootHash.size();
    if (script.end() - pc < 8)
        return error("Aux POW missing chain merkle tree size and nonce in "
                     "parent coinbase");
