...
static std::string Translate(const char *psz) {
    return QCoreApplication::translate("maza-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase,
    ...
    // Load e.g. maza_de.qm (shortcut "de" needs to be defined in
    // maza.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. maza_de_DE.qm (shortcut "de_DE" needs to be defined in
    // maza.qrc)
...
/** Class encapsulating Maza Core startup and shutdown.
...
void BitcoinApplication::handleRunawayException(const QString &message) {
    QMessageBox::critical(
        0, "Runaway exception",
        BitcoinGUI::tr("A fatal error occurred. Maza can no longer continue "
...
static void MigrateSettings() {
    ...
    static const QString legacyAppName("Maza-Qt"),
#ifdef Q_OS_DARWIN
    ...
    // We only migrate settings if we have Core settings but no Maza
    // settings
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    ...
    Q_INIT_RESOURCE(maza);
    Q_INIT_RESOURCE(maza_locale);
    ...
    try {
        ReadConfigFile(GetArg("-conf", MAZA_CONF_FILENAME));
    } catch (const std::exception &e) {
        QMessageBox::critical(
            0, QObject::tr(PACKAGE_NAME),
    ...
    // maza: links repeatedly have their payment requests routed to this
    // process:
