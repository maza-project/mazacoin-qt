...
static int AppInitRPC(int argc, char *argv[]) {
    ...
    if (argc < 2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help") ||
        ...
        if (!IsArgSet("-version")) {
            strUsage +=
                "\n" + _("Usage:") + "\n" +
                "  maza-cli [options] <command> [params]  " +
                strprintf(_("Send command to %s"), _(PACKAGE_NAME)) + "\n" +
                "  maza-cli [options] -named <command> [name=value] ... " +
                strprintf(_("Send command to %s (with named arguments)"),
                          _(PACKAGE_NAME)) +
                "\n" + "  maza-cli [options] help                " +
                _("List commands") + "\n" +
                "  maza-cli [options] help <command>      " +
    ...
    if (GetArg("-rpcpassword", "") == "") {
        ...
        if (!GetAuthCookie(&strRPCUserColonPass)) {
            throw std::runtime_error(strprintf(
                ...
                GetConfigFile(GetArg("-conf", MAZA_CONF_FILENAME))
                    .string()
                    .c_str()));
        }
    } else {
